...
### 2.4.4. Inteligencia Artificial Generativa aplicada al análisis de contratos inteligentes

#### 2.4.4.1. Descripción y contexto

La irrupción de los modelos de lenguaje de gran escala (_Large Language Models_, LLM) ha abierto una nueva línea de investigación en la auditoría de seguridad de contratos inteligentes. Herramientas como **GPT-4** (OpenAI), **Claude** (Anthropic) o **Gemini** (Google DeepMind) han demostrado capacidad para leer, comprender y razonar sobre código fuente en Solidity, lo que ha impulsado su exploración como complemento a las herramientas de análisis tradicionales [46].

A diferencia de Slither, Mythril o Echidna, los LLM no aplican técnicas formales como el análisis estático, la ejecución simbólica o el _fuzzing_. En cambio, generan respuestas a partir de patrones aprendidos durante su entrenamiento sobre grandes corpus de código y documentación de seguridad. Esta diferencia de paradigma conlleva tanto ventajas como limitaciones estructurales relevantes para su uso en auditoría.

#### 2.4.4.2. Capacidades relevantes para la auditoría

Los LLM presentan características que los hacen atractivos como herramienta de apoyo en la detección de vulnerabilidades:

- **Comprensión semántica del código:** Pueden identificar patrones de código inseguro (reentrancia, control de acceso incorrecto, uso indebido de `tx.origin`) con una explicación en lenguaje natural que facilita su interpretación por parte del auditor.
- **Bajo coste de adopción:** No requieren instalación de compiladores, entornos de análisis ni configuración de herramientas. Basta con proporcionar el código fuente como entrada al modelo.
- **Análisis contextual:** A diferencia de las herramientas formales, los LLM pueden razonar sobre la lógica de negocio del contrato, identificando fallos que dependen del contexto funcional y que las herramientas estáticas suelen ignorar.
- **Generación de informes:** Son capaces de producir explicaciones detalladas, sugerencias de remediación y clasificaciones de severidad sin necesidad de postprocesado adicional [47].

#### 2.4.4.3. Herramientas y plataformas emergentes

El potencial de los LLM ha dado lugar a plataformas especializadas que integran estos modelos en flujos de auditoría de contratos inteligentes:

- **Audit Wizard:** Plataforma que combina LLM con analizadores estáticos para generar informes automáticos de seguridad sobre contratos en Solidity [48].
- **Harvey (Trail of Bits):** Motor de _fuzzing_ asistido por IA que emplea técnicas de aprendizaje automático para guiar la generación de entradas y maximizar la cobertura de código durante las campañas de prueba [49].
- **Code4rena y plataformas de auditoría competitiva:** Algunas competiciones de auditoría de contratos inteligentes han comenzado a integrar asistentes de IA para el triaje inicial de hallazgos, actuando como primer filtro antes de la revisión manual [50].

Estudios recientes han demostrado que GPT-4 es capaz de detectar vulnerabilidades conocidas en contratos de referencia con una tasa de acierto notable, aunque con resultados heterogéneos según el tipo de vulnerabilidad y la complejidad del contrato [46], [47].

#### 2.4.4.4. Limitaciones estructurales

A pesar de su versatilidad, los LLM presentan limitaciones fundamentales que impiden su uso como herramienta de auditoría autónoma:

- **No determinismo:** El mismo contrato puede producir resultados distintos en ejecuciones diferentes, lo que impide garantizar la reproducibilidad del análisis.
- **Alucinaciones (_hallucinations_):** Los modelos pueden reportar vulnerabilidades inexistentes con argumentaciones aparentemente coherentes, generando falsos positivos difíciles de distinguir sin revisión manual exhaustiva [47].
- **Ausencia de verificación formal:** A diferencia de Mythril o Echidna, los LLM no pueden _probar_ la existencia de una vulnerabilidad; únicamente la _sugieren_ en función de patrones estadísticos aprendidos.
- **Limitación de contexto:** Los contratos complejos con múltiples archivos, librerías de herencia profunda o dependencias externas pueden superar la ventana de contexto del modelo, degradando la calidad del análisis.
- **Conocimiento estático:** Los modelos tienen una fecha de corte de entrenamiento, por lo que pueden no conocer vulnerabilidades o patrones de ataque descubiertos recientemente en el ecosistema DeFi [46].

#### 2.4.4.5. Posición respecto a las herramientas formales

El consenso emergente en la comunidad de seguridad blockchain es que los LLM no sustituyen a las herramientas de análisis formal, sino que actúan como complemento. Su valor reside principalmente en la fase de triaje inicial y en la generación de explicaciones en lenguaje natural, mientras que herramientas como Slither, Mythril o Echidna aportan garantías de reproducibilidad y rigor técnico que los modelos generativos no pueden ofrecer [48], [49].

Esta limitación estructural justifica la necesidad de soluciones como **EVMAudit**, que combinan múltiples técnicas de análisis formal en una arquitectura integrada y determinista, frente a la variabilidad inherente de los enfoques basados exclusivamente en inteligencia artificial generativa.
